...
export class AppSidebarComponent {
  ...
  // Liste finale utilisée par le template, filtrée par rôle (cf. buildVisibleNavItems) -
  // affectée dans le constructeur, une fois AuthenticationService disponible.
  navItems: NavItem[] = [];

  // Définitions brutes, non filtrées.
  private allNavItems: NavItem[] = [
    ...
    },
    {
      ...
      subItems: REFERENTIEL_CRUD_ENTITIES.map((entity) => ({
        ...
        pro: false
      })).concat([{ name: 'Grilles d’évaluation', path: '/referentiels/grilles-evaluation', pro: false }]),
    },
    {
  ...
  // "Référentiels" et "Journal d'audit" sont protégés côté route (roleGuard) - masqués ici aussi
  // pour ne pas afficher un lien que l'utilisateur ne peut de toute façon pas ouvrir. "Référentiels"
  // disparaît entièrement si aucun de ses 8 sous-référentiels n'est accessible (ex. un profil qui
  // n'a ni SADM ni ADM).
  private buildVisibleNavItems(): NavItem[] {
    return this.allNavItems
      .map((item) => this.applyRoleVisibility(item))
  ...
  private applyRoleVisibility(item: NavItem): NavItem | null {
    ...
    if (item.name === 'Référentiels') {
      ...
      if (this.authService.hasPermission('RH_EVALUATION_GERER')) {
        visibleSubItems.push({ name: 'Grilles d’évaluation', path: '/referentiels/grilles-evaluation', pro: false });
      }
